...
class SentimentDataProvider(DataProvider):
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
        ...
        data_path =r'/Users/chienyu/MLP/data-new.npz'
        loaded = np.load(data_path)
        inputs, targets = loaded['inputs'], loaded['targets']
  
        # pass the loaded data to the parent class __init__
        super(SentimentDataProvider, self).__init__(
    ...
    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(SentimentDataProvider, self).next()
        return inputs_batch, targets_batch
...
class MNISTDataProvider(DataProvider):
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, one_hot=False):
        """Create a new MNIST data provider object.
